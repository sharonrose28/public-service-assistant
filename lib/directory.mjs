...
export const serviceRecords=['service-pilot.json','services-government.json','services-civic.json','civic-extras.json','civic-contacts.json','state-portals-a.json','state-portals-b.json','ration-portals.json'].flatMap(readRecords).flatMap(record=>(record.categories||(Array.isArray(record.category)?record.category:[record.category])).map(category=>({...record,category})));
...
export function availableServices(category,{records=serviceRecords,now=new Date()}={}){
 const current=records.filter(r=>r.category===category&&isCurrent(r,now)&&r.officialPortal&&r.sourceUrls?.length);
 const superseded=new Set(current.flatMap(r=>r.supersedes||[]));
 return current.filter(r=>!superseded.has(r.jurisdictionId));
}
