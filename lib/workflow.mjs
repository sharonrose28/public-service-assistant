...
function makeDraft(r,raw){
 ...
 for(const [key,label] of [['landmark',say(lang,'Landmark','அடையாளம்','पहचान स्थल')],['ward',say(lang,'Ward','வார்டு','वार्ड')],['poleNumber',say(lang,'Pole number','கம்ப எண்','खंभा संख्या')]])if(raw[key])draft+=`\n${label}: ${raw[key]}`;
...
export async function workflow(raw,options={}){
 ...
 for(const key of ['street','landmark','ward','poleNumber'])if(raw[key]!==undefined&&(typeof raw[key]!=='string'||raw[key].length>180))throw Error('INVALID_INPUT');
 ...
 const toOption=stored=>{
  ...
  return {id:stored.id||`${stored.jurisdictionId}:${stored.category}`,stateId:stored.stateId||null,stateIds:stored.stateIds||null,cityId:stored.cityId||null,serviceScope:record.serviceScope,role:record.role||'service',department:record.department,coverage:record.coverage,description:record.description,officialPortal:record.officialPortal,sourceUrls:record.sourceUrls,lastVerifiedAt:record.lastVerifiedAt,contactEmail:verifiedContact(stored,options.now),contactPhone:verifiedPhone(stored,options.now),linkType:record.linkType||'portal',responsibility:record.responsibility||record.rights?.responsibility,escalation:record.escalation||record.rights?.escalation,conditions:[...(record.conditions||[]),...(record.exceptions||[])],steps:record.steps||[],documents:record.documents||[],requiredDetails:record.requiredDetails||[],eligibility:record.eligibility,fees:record.fees,processingTime:record.processingTime,offlineOption:record.offlineOption};
 };
 const applicable=servicesForState(r.category,r.stateId,options);
 r.officialOptions=applicable.filter(record=>record.role!=='grievance').map(toOption);
 ...
 r.hasDirectChannel=r.officialOptions.some(record=>record.linkType==='portal'&&!record.cityId&&!record.stateIds&&!record.serviceScope&&(!r.requiresState||record.stateId===r.stateId));
 ...
 r.nextAction=r.intent==='CIVIC_ISSUE'?'GENERATE_COMPLAINT':'GENERATE_MESSAGE';
