...
import {reserveAiAttempt} from './ai-budget.mjs';
...
export async function bedrockUnderstand(input,{env=process.env,fetchImpl=fetch,bedrockConverse,budgetUpdateItem,now}={}){
 if(!bedrockConfigured(env))throw Error('BEDROCK_NOT_CONFIGURED');
 await reserveAiAttempt({env,now,updateItem:budgetUpdateItem});
