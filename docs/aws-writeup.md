...
This **Build It** implementation runs on the developer's machine without an AWS account or hosted inference. The demonstrated AWS contribution is the working Strands integration. SAM files are included for optional future hosting; no live cloud deployment is claimed.
